    ...
    <section className='w-full max-w-[264px]'>
      <Sheet >
        ...
        <SheetContent side="left" className="border-none bg-white" >
          ...
          <Link href="/" className="flex cursor-ponter item-center gap-1 px-4" >
                  ...
                  return (
                    <SheetClose asChild key={item.route}>
                      <Link href={item.route} key={item.label} className={cn('mobilenav-sheet_close w-full', { 'bg-bank-gradient': isActive })}>
                          ...
                          <Image src={item.imgURL} height={20} width={20} alt={item.label} className={cn({ 'brightness-[3] invert-0': isActive })} />
